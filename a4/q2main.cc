    ...
    if(argc>4){
     ...
     switch(argc){
       case 1: voters=6;group=3;seed=getpid();break;
        case 2 :
        if(!is_number(argv[1])){ cerr<<usage<<"\n";exit(0);}
        voters=atoi(argv[1]);
        break;
        case 3 :
        ...
        case 4 :
         if(!is_number(argv[1]) or !is_number(argv[2]) or !is_number(argv[3]) or atoi(argv[3])<=0){ cerr<<usage<<"\n";exit(0);}
        voters=atoi(argv[1]); group=atoi(argv[2]);seed=atoi(argv[3]);break;
     }

     if((group%2==0 or voters%group!=0 or group>voters)){ cerr<<usage<<"\n";exit(0);}
